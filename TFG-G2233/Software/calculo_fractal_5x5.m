            ...
            for j = 1:width
                if e_indice == 0
                    ...
                    if i < height %Si (i = height), no se puede obtener i+1
                        ...
                        if j < width %Si (i = height y j= width) no se puede obtener (i+1,j+1)
                            valores_u = horzcat(valores_u , u(i+1,j+1,e_indice));
                            valores_b = horzcat(valores_b ,b(i+1,j+1,e_indice));
                            valores_u2 = horzcat(valores_u2,u2(i+1,j+1,e_indice));
                            valores_b2 = horzcat(valores_b2 , b2(i+1,j+1,e_indice));
                        end
                        if j > 1 %Si (i = height y j= 1) no se puede obtener (i+1,j-1)
                            valores_u = horzcat(valores_u , u(i+1,j-1,e_indice));
                            valores_b = horzcat(valores_b ,b(i+1,j-1,e_indice));
                            valores_u2 = horzcat(valores_u2,u2(i+1,j-1,e_indice));
                            valores_b2 = horzcat(valores_b2 , b2(i+1,j-1,e_indice));
                        end
                        if i < (height -1) %Si (i> height-1), no se puede obtener i+2
                            ...
                            if j > 2 %Si (i > height-1 y j< 2) no se puede obtener (i+2,j-2)
                                valores_u = horzcat(valores_u , u(i+2,j-2,e_indice));
                                valores_b = horzcat(valores_b ,b(i+2,j-2,e_indice));
                                valores_u2 = horzcat(valores_u2,u2(i+2,j-2,e_indice));
                                valores_b2 = horzcat(valores_b2 , b2(i+2,j-2,e_indice));
                            end
                            if j < (width -1) %Si (i > height-1 y j> width-1) no se puede obtener (i+2,j+2)
                                valores_u = horzcat(valores_u , u(i+2,j+2,e_indice));
                                valores_b = horzcat(valores_b ,b(i+2,j+2,e_indice));
                                valores_u2 = horzcat(valores_u2,u2(i+2,j+2,e_indice));
                                valores_b2 = horzcat(valores_b2 , b2(i+2,j+2,e_indice));
                    ...
                    if i > 1 %Si i = 1, no se puede obtener i-1
                        ...
                        valores_u2 = horzcat(valores_u2,u2(i-1,j,e_indice));
                        valores_b2 = horzcat(valores_b2 , b2(i-1,j,e_indice));
                        if j < width %Si (i = 1 y j= width) no se puede obtener (i-1,j+1)
                            valores_u = horzcat(valores_u , u(i-1,j+1,e_indice));
                            valores_b = horzcat(valores_b ,b(i-1,j+1,e_indice));
                            valores_u2 = horzcat(valores_u2,u2(i-1,j+1,e_indice));
                            valores_b2 = horzcat(valores_b2 , b2(i-1,j+1,e_indice));
                        end
                        if j > 1 %Si (i = 1 y j= 1) no se puede obtener (i-1,j-1)
                            valores_u = horzcat(valores_u , u(i-1,j-1,e_indice));
                            valores_b = horzcat(valores_b ,b(i-1,j-1,e_indice));
                            valores_u2 = horzcat(valores_u2,u2(i-1,j-1,e_indice));
                            valores_b2 = horzcat(valores_b2 , b2(i-1,j-1,e_indice));
                        ...
                        if i > 2 %Si (i<2), no se puede obtener i-2
                            ...
                            if j > 2 %Si (i< 2 y j< 2) no se puede obtener (i-2,j-2)
                                valores_u = horzcat(valores_u , u(i-2,j-2,e_indice));
                                valores_b = horzcat(valores_b ,b(i-2,j-2,e_indice));
                                valores_u2 = horzcat(valores_u2,u2(i-2,j-2,e_indice));
                                valores_b2 = horzcat(valores_b2 , b2(i-2,j-2,e_indice));
                            end
                            if j < (width -1) %Si (i <2 y j> width-1) no se puede obtener (i-2,j+2)
                                valores_u = horzcat(valores_u , u(i-2,j+2,e_indice));
                                valores_b = horzcat(valores_b ,b(i-2,j+2,e_indice));
                                valores_u2 = horzcat(valores_u2,u2(i-2,j+2,e_indice));
                                valores_b2 = horzcat(valores_b2 , b2(i-2,j+2,e_indice));
                    ...
                    if j < width %Si j=width, no se puede obtener j+1
                        ...
                        if j < (width -1) %Si j>width-1, no se puede obtener j+2
                            valores_u = horzcat(valores_u ,u(i,j+2,e_indice));
                            valores_b = horzcat(valores_b ,b(i,j+2,e_indice));
                            valores_u2 = horzcat(valores_u2,u2(i,j+2,e_indice));
                            valores_b2 = horzcat(valores_b2 ,b2(i,j+2,e_indice));
                        end
                    end
                    if j > 1 %Si j=1, no se puede llamar a j-1
                        ...
                        valores_u2 =horzcat(valores_u2, u2(i,j-1,e_indice));
                        valores_b2 =horzcat(valores_b2 , b2(i,j-1,e_indice));
                        if j > 2 %Si j<2, no se puede llamar a j-2
                            valores_u = horzcat(valores_u ,u(i,j-2,e_indice));
                            valores_b = horzcat(valores_b , b(i,j-2,e_indice));
                            valores_u2 =horzcat(valores_u2, u2(i,j-2,e_indice));
                            valores_b2 =horzcat(valores_b2 , b2(i,j-2,e_indice));
    ...
    %%Se calcula el volumen y area de cada capa.
       for e_indice = 1:e_maximo
        ...
        %Se definen D y D2 (en mayusculas) para mantener la nomenclatura.
            logA = log2(area(e_indice))
            logA2 = log2(area2(e_indice))
            logEpsil = log2(double(epsil));
            logE = log2(e_indice)
            
            D(e_indice -1)  = 2 - ( (logA - log2(area(epsil)) ) / (log2(e_indice) - logEpsil));
            D2(e_indice -1) = 2 - ( (logA2 - log2(area2(epsil)) ) / (log2(e_indice) - logEpsil));
        end
        D = abs(D);
        D2 = abs(D2);
