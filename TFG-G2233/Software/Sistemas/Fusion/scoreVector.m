function vector = scoreVector( img1,img2,args )
...
%   5.- normalizar o no en esta funcion. Valores 0->no y 1->si

%% Variables sigmoide

A_NCoef = 0.01;
C_NCoef = 500; % de -2 a 2
A_DCT = 0.01;
C_DCT = 500;
A_Xor = 0.01;
C_Xor = 5000; % de 0 a 13000
A_And = 0.01; 
C_And = -23000; % de -26000 a -20000
...
if (normalizar == 1)
    ...
    %distNCoef = length(find(cc > umbralCorr));  %Da valores 0 que al aplicar score ratio se va a infinito
    distNCoef = sigmf(length(find(cc > umbralCorr)), [A_NCoef C_NCoef]);
end
vector = [vector,distNCoef];
